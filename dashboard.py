            ...
            if not personas:
                ...
                for p in personas:
                    with st.expander(f"👤 {p['role']}"):
                        ...
                        with st.popover("🗑️ Delete", key=f"pop_del_{p['role']}", use_container_width=True):
            ...
            if catalog:
                ...
                for pkg in catalog:
                    ...
                    with st.container():
                        ...
                        with cols[2]:
                            if is_installed:
                                with st.popover("🗑️ Uninstall", key=f"pop_un_{pkg['id']}", use_container_width=True):
                                    st.warning(f"Uninstall {pkg['name']}?")
                                    if st.button("Confirm", key=f"confirm_un_{pkg['id']}", type="primary", use_container_width=True):
                        ...
                        if i + j < len(registry_pkgs):
                            ...
                            with cols[j].container(border=True):
                                ...
                                if st.button(
                                    ...
                                    help="Install this workflow from the community registry"
                                ):
            ...
            for ip in installed:
                with st.expander(f"📦 **{ip['name']}** v{ip['version']}", expanded=True):
                    ...
                    if ip.get("workflow"):
                        ...
                        if st.button(f"🚀 Deploy '{wf_name}' as Mission", key=f"deploy_{ip['package_id']}", use_container_width=True, help="Initialize a new mission using this workflow package"):
