...
if not selected_project:
    ...
    with st.container():
        uploaded_files = st.file_uploader(
            ...
            help="Limit: 200MB per file, 500MB total."
        )
    ...
    with tab2:
        ...
        if logs:
            for entry in reversed(logs):
                ...
                try:
                    # Attempt to format ISO timestamp for better readability
                    dt = datetime.fromisoformat(ts)
                    readable_ts = dt.strftime("%Y-%m-%d %H:%M:%S")
                except Exception:
                    readable_ts = ts

                with st.expander(f"🛠️ {entry.get('tool')} @ {readable_ts}", expanded=(entry == logs[-1])):
                    st.write("**Inputs:**")
