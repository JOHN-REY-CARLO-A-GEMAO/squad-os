...
def list_projects():
    # Optimization: Use os.scandir to reduce system calls for directory checking
...
if not selected_project:
    ...
    with tab1:
        ...
        if os.path.exists(visuals_path):
            # Optimization: Use os.scandir for more efficient file listing and filtering
            img_exts = ('.png', '.jpg', '.jpeg', '.webp')
            vid_exts = ('.mp4', '.webm')
            exts = img_exts + vid_exts

            visual_files = sorted([entry.name for entry in os.scandir(visuals_path) if entry.is_file() and entry.name.lower().endswith(exts)], reverse=True)
            if visual_files:
                cols = st.columns(2)
