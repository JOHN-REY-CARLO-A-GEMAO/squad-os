                ...
                for p in personas:
                    with st.expander(f"👤 {p['role']}"):
                        ...
                        with st.popover("🗑️ Delete", use_container_width=True):
                            ...
                            if st.button(f"Confirm Deletion", key=f"del_conf_{p['role']}", type="primary", use_container_width=True, help="Permanently delete this agent persona"):
                        ...
                        with cols[1]:
                            ...
                            if install_status == "ACTIVE":
                                ...
                                if sqad_path and sqad_path.startswith("http"):
                                    ...
                                    if st.button(f"⬇️ Install", key=f"install_{pkg['id']}", use_container_width=True, help=f"Install {pkg['name']} from {sqad_path}"):
                        ...
                        with cols[2]:
                            if is_installed:
                                with st.popover("🗑️ Uninstall", use_container_width=True):
                                    ...
                                    if st.button("Confirm", key=f"uninst_conf_{pkg['id']}", type="primary", use_container_width=True, help=f"Remove {pkg['name']} from your installed packages"):
                        ...
                        if i + j < len(registry_pkgs):
                            ...
                            with cols[j].container(border=True):
                                ...
                                if st.button(
                                    ...
                                    help=f"Ingest and install the {pkg['name']} workflow into your local store"
                                ):
            ...
            for ip in installed:
                with st.expander(f"📦 **{ip['name']}** v{ip['version']}", expanded=True):
                    ...
                    if ip.get("workflow"):
                        ...
                        if st.button(f"🚀 Deploy '{wf_name}' as Mission", key=f"deploy_{ip['package_id']}", use_container_width=True, help=f"Start a new mission using the '{wf_name}' workflow"):
        ...
        with store_tab3:
            ...
            if uploaded_sqad:
                ...
                if pkg:
                    ...
                    if st.button("💾 Install this package", use_container_width=True, type="primary", help="Install this uploaded package to the store"):
