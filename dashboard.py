...
def list_projects():
    # Optimized: Use os.scandir to reduce syscalls (only one call per directory entry)
    with os.scandir(PROJECTS_DIR) as entries:
        active = sorted([d.name for d in entries if d.is_dir()], reverse=True)
    with os.scandir(ARCHIVES_DIR) as entries:
        archived = sorted([d.name for d in entries if d.is_dir()], reverse=True)
...
if not selected_project:
    ...
    with tab1:
        ...
        if os.path.exists(visuals_path):
            ...
            # Optimized: Use os.scandir for faster file filtering
            with os.scandir(visuals_path) as entries:
                visual_files = sorted([
                    f.name for f in entries
                    if f.is_file() and f.name.lower().endswith(img_exts + vid_exts)
                ], reverse=True)
            if visual_files:
                cols = st.columns(2)
