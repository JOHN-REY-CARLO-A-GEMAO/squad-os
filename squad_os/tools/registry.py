...
# Security: Centralized dangerous Python constants
...
DANGEROUS_MODULES: Set[str] = {
    ...
    'importlib', 'builtins', 'pickle', 'ctypes', 'pty', 'marshal', 'shelve', 'urllib'
}

DANGEROUS_METHODS: Set[str] = {
    'system', 'popen', 'spawn', 'Popen', 'rmtree', 'check_output', 'check_call', 'call', 'fork', 'startfile', 'run'
}


class SecurityVisitor(ast.NodeVisitor):
    """AST visitor to robustly validate Python code for security violations."""

    def __init__(self):
        self.errors = []
        self.module_aliases = {} # Maps alias -> original module name
    ...
    def visit_Name(self, node: ast.Name):
        # Prevent assigning or referencing forbidden built-ins or dangerous modules
        if node.id in FORBIDDEN_BUILTINS:
            self.errors.append(f"Blocked: Access to forbidden built-in '{node.id}'")
        elif node.id in DANGEROUS_MODULES:
            self.errors.append(f"Blocked: Reference to dangerous module '{node.id}'")
        self.generic_visit(node)

    def visit_Attribute(self, node: ast.Attribute):
        # Check for dangerous methods (e.g. os.system)
        if isinstance(node.value, ast.Name):
            # Resolve module alias
            module_name = self.module_aliases.get(node.value.id, node.value.id)
            if module_name in DANGEROUS_MODULES and node.attr in DANGEROUS_METHODS:
                self.errors.append(f"Blocked: Use of dangerous method '{module_name}.{node.attr}'")
        self.generic_visit(node)

    def visit_Call(self, node: ast.Call):
        # Direct calls to forbidden functions
        if isinstance(node.func, ast.Name):
            if node.func.id in FORBIDDEN_BUILTINS:
                self.errors.append(f"Blocked: Forbidden built-in call '{node.func.id}()'")
        # Method calls (e.g. os.system())
        elif isinstance(node.func, ast.Attribute):
            if isinstance(node.func.value, ast.Name):
                module_name = self.module_aliases.get(node.func.value.id, node.func.value.id)
                if module_name in DANGEROUS_MODULES and node.func.attr in DANGEROUS_METHODS:
                    self.errors.append(f"Blocked: Dangerous method call '{module_name}.{node.func.attr}()'")
        self.generic_visit(node)
