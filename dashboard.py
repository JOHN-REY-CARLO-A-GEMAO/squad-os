...
if st.session_state.get("persona_deleted"):
    st.toast(f"🗑️ {st.session_state.get('persona_deleted_msg', 'Persona deleted')}")
    st.session_state.persona_deleted = False

if st.session_state.get("persona_saved"):
    st.toast(f"💾 {st.session_state.get('persona_saved_msg', 'Persona saved')}")
    st.session_state.persona_saved = False

if st.session_state.get("package_uninstalled"):
    st.toast(f"📦 {st.session_state.get('package_uninstalled_msg', 'Package uninstalled')}")
    st.session_state.package_uninstalled = False

if st.session_state.get("workflow_deployed"):
    st.toast(f"📥 {st.session_state.get('workflow_deployed_msg', 'Workflow deployed')}")
    ...
    with main_tab2:
            ...
            if not personas:
                ...
                for p in personas:
                    with st.expander(f"👤 {p['role']}"):
                        ...
                        delete_popover = st.popover(f"🗑️ Delete {p['role']}", use_container_width=True, help=f"Remove {p['role']} from the registry")
                        with delete_popover:
        ...
        with col_b:
            ...
            with st.form("new_agent_form"):
                ...
                submit_agent = st.form_submit_button("💾 Save Persona", help="Register this persona for use in future missions")
                if submit_agent:
                    if new_role and new_goal and new_backstory:
                        ...
                        st.session_state.persona_saved_msg = f"Persona '{new_role}' saved to the registry!"
                        st.rerun()
                    else:
            ...
            if catalog:
                ...
                for pkg in catalog:
                    ...
                    with st.container():
                        ...
                        with cols[2]:
                            if is_installed:
                                uninstall_popover = st.popover("🗑️ Uninstall", use_container_width=True, help=f"Remove {pkg['name']} package")
            ...
            for ip in installed:
                with st.expander(f"📦 **{ip['name']}** v{ip['version']}", expanded=True):
                    ...
                    if ip.get("workflow"):
                        ...
                        if st.button(f"🚀 Deploy '{wf_name}' as Mission", key=f"deploy_{ip['package_id']}_{wf_name}", use_container_width=True, help=f"Launch a new mission using the '{wf_name}' workflow"):
                            success = deploy_store_workflow(ip["package_id"])
                            if success:
                                st.session_state.workflow_deployed = True
                                st.session_state.workflow_deployed_msg = f"Workflow '{wf_name}' queued as a mission!"
                                st.rerun()
                            else:
