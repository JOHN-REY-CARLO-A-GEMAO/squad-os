                ...
                for p in personas:
                    with st.expander(f"👤 {p['role']}"):
                        ...
                        with st.popover(f"🗑️ Delete {p['role']}", use_container_width=True):
                            ...
                            if st.button("Confirm Delete", key=f"conf_del_{p['role']}", type="primary", use_container_width=True, help=f"Permanently remove the {p['role']} agent persona"):
                        ...
                        with cols[2]:
                            if is_installed:
                                with st.popover("🗑️ Uninstall", use_container_width=True):
                                    ...
                                    if st.button("Confirm Uninstall", key=f"conf_un_{pkg['id']}", type="primary", use_container_width=True, help=f"Remove the {pkg['name']} package from your squad"):
