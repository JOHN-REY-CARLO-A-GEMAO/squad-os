"""Mission/task/interrupt persistence — hot path for Manager + HITL.

Split from session.py (PR4). Move-only: no logic changes.
Shared event-bus infra (_event_lock, _broadcast_callbacks,
register_broadcast_callback) lives here because update_mission_snapshot
needs it and this module has no intra-package imports (no cycles).
session_memory imports the bus from here.
"""
...
# --- SHARED EVENT-BUS INFRA (used by snapshot + conversation events) ---
...
# --- HONEST FINAL STATUS (relocated from manager.py PR2b shim) ---

def compute_mission_final_status(task_states: Dict[Any, str]) -> str:
    """Honest final mission status from per-task states.

    Precedence: PAUSED_FOR_REVIEW > FAILED > COMPLETED.
    SKIPPED counts as neutral (treated like COMPLETED).
    Empty input means nothing failed -> COMPLETED.

    Note: run_mission deliberately uses a stricter unfinished-policy
    (anything not COMPLETED/SKIPPED, including PAUSED_FOR_REVIEW, fails
    the mission row) so queued missions never hang as COMPLETED while
    awaiting human approval. This helper preserves the PAUSED distinction
    for dashboard/inbox display and unit-level status reasoning.
    """
    if not task_states:
        return "COMPLETED"
    values = list(task_states.values())
    if any(v == "PAUSED_FOR_REVIEW" for v in values):
