...
if st.session_state.get("persona_created"):
    st.toast(f"Agent Persona '{st.session_state.persona_created}' saved!", icon="💾")
    st.session_state.persona_created = None

if st.session_state.get("persona_deleted"):
    st.toast("Agent Persona deleted successfully.", icon="🗑️")
    st.session_state.persona_deleted = False

if st.session_state.get("package_uninstalled"):
    st.toast("Package uninstalled successfully.", icon="📦")
    ...
    with main_tab2:
                ...
                for p in personas:
                    with st.expander(f"👤 {p['role']}"):
                        ...
                        with st.popover(f"🗑️ Delete {p['role']}", use_container_width=True, help=f"Delete the '{p['role']}' persona"):
                            ...
                            if st.button("Confirm Delete", key=f"conf_del_{p['role']}", type="primary", use_container_width=True, help=f"Confirm deletion of {p['role']}"):
        ...
        with col_b:
            ...
            with st.form("new_agent_form"):
                ...
                submit_agent = st.form_submit_button("💾 Save Persona", help="Save this persona to the registry")
                if submit_agent:
            ...
            if catalog:
                ...
                for pkg in catalog:
                    ...
                    with st.container():
                        ...
                        with cols[2]:
                            if is_installed:
                                with st.popover("🗑️ Uninstall", use_container_width=True, help=f"Uninstall {pkg['name']}"):
                                    st.warning(f"Are you sure you want to uninstall {pkg['name']}?")
                                    if st.button("Confirm Uninstall", key=f"conf_uninst_{pkg['id']}", type="primary", use_container_width=True, help=f"Confirm uninstallation of {pkg['name']}"):
