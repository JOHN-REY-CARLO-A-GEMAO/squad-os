...
def load_missions():
    # Optimization: Using standard lists/dicts instead of pandas to reduce memory overhead
    # and improve startup time in high-frequency (5s) auto-refreshing dashboard.
    ...
    if conn:
        try:
            ...
            conn.close()
            # Convert sqlite3.Row objects to dictionaries for easy access
    ...
    def _render_chat():
        ...
        with chat_container:
            if selected_id is not None:
                mission_row = next((m for m in missions_list if m["id"] == selected_id), None)
        ...
        with st.container():
            ...
            if selected_id is not None:
                ...
                is_active = False
                if mission_row:
                    s = mission_row.get("status", "").upper()
