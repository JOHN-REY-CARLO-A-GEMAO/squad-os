    ...
    with main_tab2:
                ...
                for p in personas:
                    with st.expander(f"👤 {p['role']}"):
                        ...
                        with st.popover(f"🗑️ Delete {p['role']}", use_container_width=True, key=f"pop_del_{p['role']}"):
                            st.warning(f"Are you sure you want to delete this persona?")
                            if st.button("Confirm Delete", key=f"confirm_del_{p['role']}", type="primary", use_container_width=True):
        ...
        with col_b:
            ...
            with st.form("new_agent_form"):
                ...
                submit_agent = st.form_submit_button("💾 Save Persona", help="Persist this agent configuration to the local registry")
                if submit_agent:
                    ...
                    with st.container():
                        ...
                        with cols[1]:
                            ...
                            if install_status == "ACTIVE":
                                ...
                                if st.button(f"⬇️ Get from Registry", key=f"remote_{pkg['id']}", use_container_width=True, help="Download and install this package from the community registry"):
                                ...
                                if sqad_path and sqad_path.startswith("http"):
                                    ...
                                    if st.button(f"⬇️ Install", key=f"install_{pkg['id']}", use_container_width=True, help="Install this local package to your squad"):
                        ...
                        with cols[2]:
                            if is_installed:
                                with st.popover("🗑️ Uninstall", use_container_width=True, key=f"pop_uninst_{pkg['id']}"):
                                    st.warning("Are you sure you want to uninstall this package?")
                                    if st.button("Confirm Uninstall", key=f"confirm_uninst_{pkg['id']}", type="primary", use_container_width=True):
                        ...
                        if i + j < len(registry_pkgs):
                            ...
                            with cols[j].container(border=True):
                                ...
                                if st.button(
                                    ...
                                    help="Download and install this validated workflow"
                                ):
            ...
            for ip in installed:
                with st.expander(f"📦 **{ip['name']}** v{ip['version']}", expanded=True):
                    ...
                    if ip.get("workflow"):
                        ...
                        if st.button(f"🚀 Deploy '{wf_name}' as Mission", key=f"deploy_{ip['package_id']}", use_container_width=True, help="Queue this workflow for execution by the squad"):
