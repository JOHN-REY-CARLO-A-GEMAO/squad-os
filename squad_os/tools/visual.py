    ...
    async def execute(self, action: str, url: str = None, selector: str = None, text: str = None, description: str = None, wait_for: str = None) -> str:
        try:
            ...
            if action == "navigate":
                ...
                # Security Check: Prevent Local File Disclosure (LFD) and SSRF by restricting protocols
                parsed_url = urlparse(url)
                if parsed_url.scheme not in ["http", "https"]:
                    return f"Error: Access denied. Protocol '{parsed_url.scheme}' is not allowed. Only HTTP/HTTPS is permitted."
