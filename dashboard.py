...
def list_projects():
    # Optimization: Use os.scandir to reduce system calls by retrieving file info during directory listing
...
if not selected_project:
    ...
    with tab1:
        ...
        if os.path.exists(visuals_path):
            ...
            # Optimization: Use os.scandir for faster filtering and O(N) traversal
            with os.scandir(visuals_path) as entries:
                visual_files = sorted(
                    [e.name for e in entries if e.is_file() and e.name.lower().endswith(img_exts + vid_exts)],
                    reverse=True
                )
            if visual_files:
                cols = st.columns(2)
