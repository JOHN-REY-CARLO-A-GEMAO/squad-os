    ...
    with main_tab2:
            ...
            if not personas:
                ...
                for p in personas:
                    with st.expander(f"👤 {p['role']}"):
                        ...
                        with st.popover("🗑️ Delete", use_container_width=True, help=f"Remove the '{p['role']}' persona from registry", key=f"pop_del_{p['role']}"):
                            st.warning(f"Are you sure you want to delete the '{p['role']}' persona?")
                            if st.button(f"Confirm Deletion", key=f"btn_del_{p['role']}", type="primary", use_container_width=True):
        ...
        with col_b:
            ...
            with st.form("new_agent_form"):
                ...
                submit_agent = st.form_submit_button("💾 Save Persona", help="Register this new agent persona to the squad")
                if submit_agent:
            ...
            if catalog:
                ...
                for pkg in catalog:
                    ...
                    with st.container():
                        ...
                        with cols[1]:
                            ...
                            if install_status == "ACTIVE":
                                ...
                                if sqad_path and sqad_path.startswith("http"):
                                    ...
                                    if st.button(f"⬇️ Install", key=f"install_{pkg['id']}", use_container_width=True, help=f"Install '{pkg['name']}' from local source"):
                        ...
                        with cols[2]:
                            if is_installed:
                                with st.popover("🗑️ Uninstall", use_container_width=True, help=f"Remove '{pkg['name']}' from installed packages", key=f"pop_uninst_{pkg['id']}"):
                                    st.warning(f"Are you sure you want to uninstall '{pkg['name']}'?")
                                    if st.button("Confirm Uninstall", key=f"btn_uninst_{pkg['id']}", type="primary", use_container_width=True):
                        ...
                        if i + j < len(registry_pkgs):
                            ...
                            with cols[j].container(border=True):
                                ...
                                if st.button(
                                    ...
                                    help=f"Download and install the '{pkg['name']}' workflow from the community registry"
                                ):
            ...
            for ip in installed:
                with st.expander(f"📦 **{ip['name']}** v{ip['version']}", expanded=True):
                    ...
                    if ip.get("workflow"):
                        ...
                        if st.button(f"🚀 Deploy '{wf_name}' as Mission", key=f"deploy_{ip['package_id']}", use_container_width=True, help=f"Execute the '{wf_name}' workflow"):
        ...
        with store_tab3:
            ...
            if uploaded_sqad:
                ...
                if pkg:
                    ...
                    if st.button("💾 Install this package", use_container_width=True, type="primary", help="Install the uploaded .sqad package into the Store"):
