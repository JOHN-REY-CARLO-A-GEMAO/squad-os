...
def _validate_terminal_command(command: str, workspace: Optional[str] = None) -> tuple[bool, str]:
    ...
    try:
        ...
        lexer.wordchars += "./-" # Allow common path characters in tokens
        tokens = list(lexer)
    ...
    COMMAND_OPERATORS = {';', '&&', '||', '|', '&'}
    # Redirection operators (don't necessarily start a new command, but follow them)
    REDIRECT_OPERATORS = {'>', '<', '>>', '>&', '<<', '<&'}
    ...
    for i, token in enumerate(tokens):
        if is_new_command:
            ...
            if base_cmd not in ALLOWED_COMMANDS and not base_cmd.startswith('./'):
                # Check if it's a path and if the basename is allowed
                if '/' in base_cmd:
        ...
        elif token in COMMAND_OPERATORS:
            is_new_command = True
        elif token in REDIRECT_OPERATORS:
            # Next token after redirection might be a filename, not a command
            pass
        elif workspace:
            # Potential path argument validation
            # Check if this token looks like a path and if it's safe
            if '/' in token or token.startswith('..'):
                # We only check if it's absolute or has traversal
