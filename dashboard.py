...
if st.session_state.get("persona_created"):
    st.toast("💾 Persona created successfully!")
            ...
            if not personas:
                ...
                for p in personas:
                    with st.expander(f"👤 {p['role']}"):
                        ...
                        with st.popover(f"🗑️ Delete {p['role']}", use_container_width=True, help=f"Remove the {p['role']} persona from the registry"):
                            st.warning(f"Are you sure you want to delete the '{p['role']}' persona? This cannot be undone.")
                            if st.button(f"Confirm Deletion", key=f"conf_del_{p['role']}", type="primary", use_container_width=True):
                        ...
                        with cols[2]:
                            if is_installed:
                                with st.popover("🗑️ Uninstall", use_container_width=True, help=f"Remove the {pkg['name']} package"):
                                    st.warning(f"Uninstall {pkg['name']}? This will remove its tools and workflows.")
