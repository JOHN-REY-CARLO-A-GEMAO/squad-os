...
def list_projects():
    ...
    # to reduce system calls by fetching entry metadata (is_dir) during iteration.
...
if not selected_project:
    ...
    with tab1:
        ...
        if os.path.exists(visuals_path):
            # Optimization: Use os.scandir() to retrieve filenames and metadata in one go.
            ...
            with os.scandir(visuals_path) as it:
                visual_files = sorted(
                    [entry.name for entry in it if entry.is_file() and entry.name.lower().endswith(allowed_exts)],
                    reverse=True
                )
            if visual_files:
                cols = st.columns(2)
