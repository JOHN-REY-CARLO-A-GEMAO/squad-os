...
def list_projects():
    # Use os.scandir within a context manager for better performance and resource safety
    with os.scandir(PROJECTS_DIR) as it:
        active = sorted([d.name for d in it if d.is_dir()], reverse=True)
    with os.scandir(ARCHIVES_DIR) as it:
        archived = sorted([d.name for d in it if d.is_dir()], reverse=True)
...
if not selected_project:
    ...
    with tab1:
        ...
        if os.path.exists(visuals_path):
            ...
            # Use os.scandir within a context manager for efficient file listing and resource safety
            with os.scandir(visuals_path) as it:
                visual_files = sorted([f.name for f in it if f.is_file() and f.name.lower().endswith(exts)], reverse=True)
            if visual_files:
                cols = st.columns(2)
