...
def load_missions():
    """Fetch mission metadata for the selector. Selective loading for performance."""
    conn = get_db_connection()
    if conn:
        try:
            cursor = conn.cursor()
            # Fetch only what's needed for the sidebar/selector to minimize I/O
            cursor.execute("SELECT id, goal, status, uploaded_files FROM missions ORDER BY id ASC")
...
def load_mission_details(mission_id):
    """Fetch full mission details including heavy conversation history on-demand."""
    conn = get_db_connection()
    if conn:
        try:
            cursor = conn.cursor()
    ...
    def _render_chat():
        ...
        with chat_container:
            if selected_id is not None:
                # Use load_mission_details for lazy-loading heavy conversation data
        ...
        with st.container():
            ...
            if selected_id is not None:
                # Find current row in metadata list to check status
