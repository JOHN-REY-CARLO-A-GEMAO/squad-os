...
def list_projects():
    # Use os.scandir for O(N) traversal, significantly faster than listdir + isdir (O(2N))
    # especially with many projects as cached DirEntry objects avoid extra stat calls.
    active = sorted([entry.name for entry in os.scandir(PROJECTS_DIR) if entry.is_dir()], reverse=True)
    archived = sorted([entry.name for entry in os.scandir(ARCHIVES_DIR) if entry.is_dir()], reverse=True)
...
if not selected_project:
    ...
    with tab1:
        ...
        if os.path.exists(visuals_path):
            ...
            # Use os.scandir for faster file filtering in high-refresh dashboard
            visual_files = sorted(
                [entry.name for entry in os.scandir(visuals_path)
                 if entry.is_file() and entry.name.lower().endswith(img_exts + vid_exts)],
                reverse=True
            )
            if visual_files:
                cols = st.columns(2)
