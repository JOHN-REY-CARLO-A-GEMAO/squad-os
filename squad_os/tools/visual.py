    ...
    async def execute(self, action: str, url: str = None, selector: str = None, text: str = None, description: str = None, wait_for: str = None) -> str:
        try:
            ...
            if action == "navigate":
                ...
                # Security: Restrict to http/https protocols to prevent SSRF and local file disclosure
                parsed_url = urlparse(url)
                if parsed_url.scheme.lower() not in ["http", "https"]:
                    return f"Error: Access denied. Protocol '{parsed_url.scheme}' is not allowed. Only http and https are permitted."
