...
for flag, msg, icon in [
    ("mission_submitted", "Mission dispatched successfully!", "🚀"),
    ("persona_deleted", "Persona deleted successfully", "🗑️"),
    ("persona_created", "Persona saved successfully", "💾"),
    ("package_uninstalled", "Package uninstalled successfully", "📦"),
    ("workflow_deployed", "Workflow deployed as mission", "🚀")
]:
            ...
            if not personas:
                ...
                for p in personas:
                    with st.expander(f"👤 {p['role']}"):
                        ...
                        with st.popover("🗑️", help=f"Delete {p['role']}", use_container_width=True):
            ...
            if catalog:
                ...
                for pkg in catalog:
                    ...
                    with st.container():
                        ...
                        with cols[2]:
                            if is_installed:
                                with st.popover("🗑️", help=f"Uninstall {pkg['name']}", use_container_width=True):
