...
def load_missions():
    """
    Optimized: Returns missions as a list of dicts instead of a pandas DataFrame.
    Reduces memory footprint and overhead for the 5s auto-refresh cycle.
    """
    conn = get_db_connection()
    if conn:
        try:
            # get_db_connection already sets row_factory = sqlite3.Row
            cursor = conn.cursor()
