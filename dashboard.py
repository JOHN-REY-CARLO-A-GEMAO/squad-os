...
def list_projects():
    ...
    # to avoid redundant stat() calls. Faster for directories with many projects.
...
if not selected_project:
    ...
    with tab1:
        ...
        if os.path.exists(visuals_path):
            ...
            vid_exts = ('.mp4', '.webm')
            target_exts = img_exts + vid_exts

            # Optimization: Use os.scandir() for more efficient file listing and attribute access
            with os.scandir(visuals_path) as it:
                visual_files = sorted(
                    [entry.name for entry in it if entry.is_file() and entry.name.lower().endswith(target_exts)],
                    reverse=True
                )
            if visual_files:
                cols = st.columns(2)
