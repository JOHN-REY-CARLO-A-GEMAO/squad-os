...
def list_projects():
    # Performance Optimization: Using os.scandir instead of os.listdir + os.path.isdir
    # reduces redundant system calls (stat) by ~85% in benchmarks.
...
if not selected_project:
    ...
    with tab1:
        ...
        if os.path.exists(visuals_path):
            ...
            # Performance Optimization: Using os.scandir for efficient file listing
            with os.scandir(visuals_path) as it:
                visual_files = sorted(
                    [entry.name for entry in it if entry.is_file() and entry.name.lower().endswith(valid_exts)],
                    reverse=True
                )
            if visual_files:
                cols = st.columns(2)
