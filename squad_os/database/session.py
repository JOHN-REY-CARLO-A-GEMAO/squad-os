...
async def _run_migrations(db: aiosqlite.Connection, current_version: int) -> int:
    ...
    migrations = {
        ...
        # Performance indexes for status lookups and mission-task joins
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # We run this after all core tables are created to ensure migrations
        # targeting any table (e.g. tasks) succeed.
        async with db.execute("PRAGMA user_version") as cursor:
            user_version = (await cursor.fetchone())[0]

        # Trigger migrations if there are any new ones
