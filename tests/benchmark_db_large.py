...
async def setup_data():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        print("Inserting 100,000 missions...")
        missions = [
        ...
        tasks = [
            (i % 100000 + 1, f"Task {i}", "Agent", "PENDING" if i % 5 == 0 else "COMPLETED")
            for i in range(100000)
        ]
        await db.executemany(
...
async def run_queries():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        start = time.perf_counter()
        async with db.execute("SELECT * FROM tasks WHERE mission_id = ?", (50000,)) as cursor:
            rows = await cursor.fetchall()
        end = time.perf_counter()
        print(f"Query tasks by mission_id (50000): {(end - start) * 1000:.2f}ms (found {len(rows)} rows)")
